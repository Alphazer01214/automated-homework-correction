  ...
  <div>
    ...
    <ul v-else-if="historyList.length > 0">
      <li v-for="item in historyList" :key="item.id">
        ...
        <ul v-for="(question, question_index) in item.questions" :key="question_index">
          题目{{question_index}}: [question] {{question.question}} - [student-answer] {{question.student_answer}} - [model-feedback] {{question.model_feedback}}
        </ul>
        <button @click="assignmentStore.showDetailByIndex(item_index)">查看作业{{item_index}}详情</button>
      </li>
    </ul>
    <div v-else>无记录。</div>
  </div>
